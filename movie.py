...
def getMovieList(url):
	...
	html = etree.HTML(r.text)
	lis = html.xpath("//*[@class='me1 clearfix']/li");
	return lis;

def getMovieDetail(url):
	lis = getMovieList(url)
	for li in lis:
		score 	= li.xpath("a/span/text()") #评分
		href 	= li.xpath("a/@href") #链接
		title 	= li.xpath("h3/a/text()") #标题
		detail 	= li.xpath("span/text()") #简介
		img		= li.xpath("a/img/@_src") #封面
		try:
			mid = href[0].split("/")[-1]
